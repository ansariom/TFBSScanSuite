 ...
 * @version 1.0
 */
public class GenFeatures_Jason {

    private static class RefData {
        public String id = null;
        public int start = -1;

        public RefData(String id, int start) {
            this.id = id;
            this.start = start;
        }
    }
     ...
     */
    public GenFeatures_Jason() {
    }

    public static void main(String[] args) throws java.io.IOException, java.lang.InterruptedException {
        GenFeatures_Jason ss = new GenFeatures_Jason();
        ...
        // Get labels for seqs
        HashMap <String, RefData> fastaCoords = new HashMap <String, RefData>();
        ...
        for (int i = 0; i < seqLabels.length; i++) {
            ...
            if (map_Fname != null) {
                ...
                try {
                    ...
                    RefData refData = new RefData(ref, start);
        ...
        for (int i = 0; i < S.length; i++) {
			...
			for (int p = 0; p < posArr.length; p++) {
				TagScoresRunner run = new TagScoresRunner(S[i], Scores, nucsAfterTSS, BG_WIN,
                ...
                if (map_Fname != null) {
                    RefData refData = fastaCoords.get(seqName);
                    for(int wv = 0; wv < nWinVars; wv++) {
                        ...
                        int tssLoc = S[i].length - nucsAfterTSS;
