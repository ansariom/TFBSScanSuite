...
public class GenFeatures_byMax {
    ...
    public static String maxScoreFile = null;
     ...
     */
    public GenFeatures_byMax() {
		// TODO Auto-generated constructor stub
	}
    public static void main(String[] args) throws java.io.IOException, java.lang.InterruptedException {
        GenFeatures ss = new GenFeatures();
        ...
        // Mitra - Read max possible score for each PWM 
        if (maxScoreFile != null) {
        	maxScores = Load.loadDoubleColumnFile(new File(maxScoreFile));
        } else {
        	// No threshold was given - assume that threshold will be zero
        	maxScores = new DoubleColReturn(pwms.labels);
        }
        ...
        for (int i = 0; i < S.length; i++) {
			...
			for (int p = 0; p < posArr.length; p++) {
				TagScoresRunner run = new TagScoresRunner(S[i], Scores, nucsDownStream, BG_WIN,
					...
					posArr, i, p, flankingFeatureWindowLength, numFlankingWindows, maxScores);
                ...
                if (map_Fname != null) {
                    ...
                    for(int wv = 0; wv < nWinVars; wv++) {
                        ...
                        int tssLoc = S[i].length - nucsDownStream; // for + strand 
                       	int winL = winVarL[wv] + posArr[p] + tssLoc; 
    ...
    private static void parseArgs(String[] args) {
        ...
        try {
            ...
            if (cmdLine.hasOption("help")) {
                ...
                if (cmdLine.hasOption("maxScores")) {
                	maxScoreFile = cmdLine.getOptionValue("maxScores");
                }
        ...
        if (help) {
            ...
            String cmdLineSyntax = "java -jar GenFeaturesByMax tfbs_llscan.jar <FWD_ROEs.table> <REV_ROEs.table> <sequence.fasta> <PWM_FILE> <Outfile.rdat>";
    ...
    private static Options buildOptions() {
        ...
        options.addOption(Option.builder()
        		...
        		.hasArg(true)
        		.argName("FILENAME")
        		.desc("file to read max possible loglik scores for each PWM (default: all scores set to 0)")
        		.required(false)
        		.build());
    ...
    private static class TagScoresRunner implements Callable<TagScoreResult>{
		...
		public TagScoresRunner(char[] s, DoubleColReturn scores,
				...
				int i, int p, int flankingFeatureWindowLength, int numFlankingWindows, DoubleColReturn maxScores) {
			super();
			S = s;
			Scores = scores;
                ...
                for (int flankWin = -numFlankingWindows; flankWin <= numFlankingWindows; flankWin++) {
                    ...
                    if (!byNT) {
                        featureVals[featureIndex] = cumScoreInWindow(pos + L + flankingPos, pos + R + flankingPos, S, ThetaT, strand,
                                B, B_M1, Scores.values[pwmIdx], nucsAfterTSS, maxScores.values[pwmIdx]);
    ...
    public static double cumScoreInWindow(int L, int R, char[] S, double[][] ThetaT, String strand, double[][] B, double[][][] B_M1, double Score, int beyondTSS, double maxPWMScore) throws BadCharException {
        int w = ThetaT.length;
        int chrInd, chrInd_prev;
